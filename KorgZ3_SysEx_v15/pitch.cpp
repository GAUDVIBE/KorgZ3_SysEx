...
void pitchUpdate() {
  ...
  // --- DEBUG TEMPORAIRE : releve de la course reelle et de la courbe ---
  {
    static int wmin = 1023, wmax = 0;
    static unsigned long dbg = 0;
    if (filteredRaw < wmin) wmin = filteredRaw;
    if (filteredRaw > wmax) wmax = filteredRaw;
    if (millis() - dbg >= 100) {
      dbg = millis();
      Serial.print(F("WHEEL|raw=")); Serial.print(filteredRaw);
      Serial.print(F(" centre="));   Serial.print(centerRaw);
      Serial.print(F(" val14="));    Serial.print(value14);
      Serial.print(F(" min="));      Serial.print(wmin);
      Serial.print(F(" max="));      Serial.println(wmax);
    }
  }
