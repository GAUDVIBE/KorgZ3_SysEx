 ...
 *
 * ┌──────────────────────────────────────────────────────────────────────┐
 * │ CARTE v1.5 — brochage DROIT. Le jumeau de ce croquis, dans le dossier │
 * │ KorgZ3_SysEx_26-07-2026, vise la carte v1.2 dont les broches sont     │
 * │ inversées. NE PAS INTERVERTIR : sur une v1.2, la page V mettrait le   │
 * │ plan de masse en sortie à l'état haut et détruirait la broche.        │
 * └──────────────────────────────────────────────────────────────────────┘
 *
 * Boutons de page (un par page, accès direct) : 14=I, 7=II, 8=III, 6=IV, 5=V
 * LEDs de page  : 15=I, 12=II, 11=III, 10=IV, 9=V — celle de la page courante
 *                 reste allumée en permanence.
 * Pin 16 : appui long ≥3s → Dump Request au Z3 (LED 17 clignote)
 * Pin 18 : appui court → naviguer dans les dumps mémorisés
 *          appui long ≥3s → sauvegarder le dump en cours (RAM + EEPROM)
 * LED 19 : confirmation de sauvegarde / chargement
 *
...
#define OLED_RESET    -1
Ecran display;   // I2C logiciel sur les vraies SDA/SCL (20/21) — voir oled.h
...
// CARTE v1.5 — le brochage du shield est droit (voir board_map.h), donc les
// numeros du schema sont utilises tels quels. Les cinq paires bouton+LED sont
// rendues a leur fonction : un bouton par page, la LED de la page courante
// allumee en permanence. Tous les contournements de la v1.2 ont disparu —
// aucune broche de secours, aucun code binaire de page, aucune broche interdite.
const byte PAGE_BTN[NUM_PAGES] = {megaDigital(14), megaDigital(7),
                                  megaDigital(8),  megaDigital(6),
                                  megaDigital(5)};
const byte PAGE_LED[NUM_PAGES] = {megaDigital(15), megaDigital(12),
                                  megaDigital(11), megaDigital(10),
                                  megaDigital(9)};

// COLONNE DE GAUCHE — 2 paires de fonction.
// Le Dump Request redevient operant : le MIDI IN de la v1.2 etait perdu parce
// que le net D0_RX du shield tombait sur la broche 7 du Mega, qui n'est pas une
// broche d'UART. Sur la v1.5 il tombe sur la vraie RX0 (voir midi_port.h).
const int BTN_DUMP     = megaDigital(16);
const int LED_DUMP_REQ = megaDigital(17);
...
// Appelee par Ecran::display() entre deux tranches du transfert I2C, et depuis
// le dessin (voir oled.h). pitchUpdate() ne touche jamais a l'affichage — elle
// depose ses evenements, que loop() consomme apres coup — donc aucune recursion
// possible ; le garde-fou ci-dessous rend cette propriete explicite.
void ecranPompe() {
...
void updateDisplay() {
  ...
  if (sendBufferSize > 0) {
    ...
    // autant que trois periodes de molette. On rend la main a chaque cellule,
    // comme entre les tranches du transfert.
    ecranPompe();
    for (int i = 0; i < 4; i++) {
      ecranPompe();
      int idx = startIdx + i;
...
// Affiche la page courante : une LED par page, seule celle de la page courante
// est allumee. C'est le comportement d'origine, rendu possible par le brochage
// correct de la v1.5 — la v1.2 devait le remplacer par un code binaire sur les
// trois seules LED qu'elle savait piloter.
void majLedsPages() {
  for (byte p = 0; p < NUM_PAGES; p++) {
    digitalWrite(PAGE_LED[p], (p == currentPage) ? HIGH : LOW);
  }
}
...
void receiveMidi() {
  while (MIDI_PORT.available()) {
    ...
    DBG.print(F("RX|")); DBG.println(b, HEX);  // DEBUG
    if (b == 0xF0) {
...
// =================== SETUP ===================
void setup() {
  // Serial0 porte le MIDI sur cette carte : les deux usages s'excluent.
#if DEBUG_USB
  DEBUG_USB_BEGIN();            // 115200 — le MIDI est sacrifie, voir debug_usb.h
#else
  MIDI_PORT.begin(MIDI_BAUD);   // 31250 — fonctionnement normal
#endif
  delay(500);
  ...
  // Bouton cycle = BTN_PRESET (broche 18)
  ...
  // Bouton Dump Request = BTN_DUMP (broche 16)
  ...
  if (OLED_UTILISABLE) {
    ...
    DBG.print(F("oled_begin=")); DBG.println(oledOk ? 1 : 0);
...
void loop() {
  ...
  switch (pitchTakeEvent()) {
    case PITCH_EVENT_CONNECTED:
      DBG.print(F("DBG|t=")); DBG.print(millis()); DBG.println(F(" pitch=CONNECTED"));  // DEBUG TEMPORAIRE
      showMessage("Molette OK", "Centre calibre");
      delay(1000);
      updateDisplay();
      break;
    case PITCH_EVENT_DISCONNECTED:
      DBG.print(F("DBG|t=")); DBG.print(millis()); DBG.println(F(" pitch=DISCONNECTED"));  // DEBUG TEMPORAIRE
      showMessage("Molette", "Debranchee");
      delay(1000);
      updateDisplay();
      break;
    case PITCH_EVENT_RANGE: {
      DBG.print(F("DBG|t=")); DBG.print(millis()); DBG.println(F(" pitch=RANGE"));  // DEBUG TEMPORAIRE
  ...
  // --- DEBUG TEMPORAIRE : etat brut toutes les 500ms, a retirer apres diagnostic ---
  {
    static unsigned long dbgLast = 0;
    if (millis() - dbgLast >= 500) {
      dbgLast = millis();
      DBG.print(F("DBG|t=")); DBG.print(millis());
      DBG.print(F(" pot16=")); DBG.print(muxRead(0));
      DBG.print(F(" sw=")); DBG.print(muxRead(2));
      DBG.print(F(" wheel=")); DBG.print(muxRead(1));
      DBG.print(F(" btnPage="));
      for (int i = 0; i < NUM_PAGES; i++) { DBG.print(digitalRead(PAGE_BTN[i])); }
      DBG.print(F(" dump=")); DBG.print(digitalRead(BTN_DUMP));
      DBG.print(F(" preset=")); DBG.print(digitalRead(BTN_PRESET));
      DBG.print(F(" page=")); DBG.print(currentPage);
      DBG.print(F(" slot=")); DBG.print(currentSlot);
      DBG.print(F("/")); DBG.print(presetCount);
      DBG.println();
    }
  }

  // Le bloc « bouton PAGE SUIVANTE » de la v1.2 a ete retire ici : il pilotait
  // PAGE_NEXT_BTN, qui n'etait que le bouton du Dump Request detourne faute de
  // MIDI IN. Sur la v1.5 les cinq boutons de page fonctionnent, et le bouton 16
  // est rendu au Dump Request, gere plus bas par DUMP_BTN_PIN.
